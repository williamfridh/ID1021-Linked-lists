class LinkedListSingly {
	...
	public class Cell {
		...
		Cell(int target_val, LinkedListSingly list) {
			this.val = target_val;
			this.nxt = list.first;
		}
	 ...
	 */
	public static LinkedListSingly generate(int size, int startAt) {
	 ...
	 */
	public static LinkedListSingly generateSimple(int size) {
		LinkedListSingly list = new LinkedListSingly();
		for (int i = 0; i < size; i++) {
			list.add(i);
		}
	 ...
	 * Append
	 */
	public void append(LinkedListSingly second_list) {
		Cell last = this.first;
		while (last.nxt != null) {
			last = last.nxt;
		}
