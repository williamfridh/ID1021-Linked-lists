...
public class Benchmark {
	...
	public static void main(String[] args) {
		...
		LinkedListSingly lst = new LinkedListSingly();
		lst.add(1);
		lst.add(2);
		lst.add(3);
		System.out.println(lst.toString());
		LinkedListSingly.Cell tmp = lst.first;
		tmp.nxt.nxt.unlink();
		
		System.out.println(lst.toString());

		/*
		LinkedListDoubly list_a = LinkedListDoubly.generate(10,0);
		LinkedListDoubly list_b = LinkedListDoubly.generate(10,10);
	...
	private static void benchmarkOne(int arr_a_size, int[] arr_b_size, int tries) {
		...
		for (int arr_size : arr_b_size) {
			...
			for (int i = 0; i < tries; i++) {
				...
				// Warm-up
				LinkedListDoubly list_a = LinkedListDoubly.generate(arr_a_size, 0);
				LinkedListDoubly list_b = LinkedListDoubly.generate(arr_size, arr_a_size);
				list_a.append(list_b);

				// Meassuring
				list_a = LinkedListDoubly.generate(arr_a_size, 0);
				list_b = LinkedListDoubly.generate(arr_size, arr_a_size);
	...
	private static void benchmarkTwo(int arr_a_size, int[] arr_b_size, int tries) {
		...
		for (int arr_size : arr_b_size) {
			...
			for (int i = 0; i < tries; i++) {
				...
				// Warm-up
				LinkedListDoubly list_a = LinkedListDoubly.generate(arr_size, arr_a_size);
				LinkedListDoubly list_b = LinkedListDoubly.generate(arr_a_size, 0);
				list_a.append(list_b);

				// Meassuring
				list_a = LinkedListDoubly.generate(arr_size, arr_a_size);
				list_b = LinkedListDoubly.generate(arr_a_size, 0);
				double n0 = System.nanoTime();
				list_a.append(list_b);
				double n1 = System.nanoTime();
				double n = n1 - n0;
	...
	private static void benchmarkFour(int[] sizes, int tries) {
		...
		for (int size : sizes) {
			...
			for (int i = 0; i < tries; i++) {
				...
				// Warm-up
				LinkedListDoubly list_a = LinkedListDoubly.generate(size, 0);

				// Meassuring
				double n0 = System.nanoTime();
				list_a = LinkedListDoubly.generateSimple(size);
				double n1 = System.nanoTime();
				double n = n1 - n0;
				if (n < best_list) {
					best_list = n;
